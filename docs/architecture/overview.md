...
The browser sends a proposed review response. The `submit_review` database function authenticates the caller, locks the user's review stream, checks word/state versions, calculates correctness and due time, appends the event, updates the per-word state, and adjusts the user's adaptive ability in one transaction. RLS protects direct table reads and RPCs are explicitly granted. Shared card difficulty stays fixed; `user_ability` controls the next personalized catalog sample.
