...
Track rules use a composite key `(catalog_id, track_id)` with role, priority, positive weight, rationale and timestamps. Current rules are transparent editorial defaults. The weights are relative ranking multipliers, not probabilities or daily quotas. Each of the 350 cards has one rule for each of the three tracks (1,050 rules); optional cards are fallback candidates, not a requirement to study every level.
...
Referenced catalog cards cannot be hard-deleted (`ON DELETE RESTRICT`). Retire them with `is_active=false`; personal snapshots and review history remain readable. A foreign key is an identity constraint, not active-card validation: a future import action must select the active card under authenticated RLS before copying it. Atomic import/recommendation RPCs are future work.

All four shared tables have RLS. Authenticated users can read levels, active cards, active tracks, and rules whose parents are active. Anonymous reads and ordinary authenticated writes are denied. Maintenance uses migrations or an authorized server-only administrative connection. No new privileged function is exposed to clients. The existing user ownership policies and column grants continue protecting personal vocabulary.

## Validation

`npm test -- tests/catalog.test.ts` executes the actual v1, catalog, seed and exp1 migrations in the installed PGlite PostgreSQL runtime. It checks 350 cards, 50 per level, no duplicate normalized term/meaning pairs, 1,050 track rules, required fields, all three tracks, valid roles, RLS/grants, retirement, snapshot preservation, duplicate imports, and the original review RPC's idempotency/stale-version behavior using synthetic users. Expansion checks cover exactly 175 additions, deterministic IDs without v1 collisions, level metadata, provenance, unchanged existing shared rows, and the original editorial role/domain logic. Both content migrations insert only shared data. It runs in `npm run verify` through the existing test command.
